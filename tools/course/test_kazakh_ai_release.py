...
class KazakhAIReleaseTest(unittest.TestCase):
    def test_third_block_is_trilingual_and_transactional(self):
        sql, expected = release.prepare()
        self.assertEqual(len(expected), 48)
        self.assertEqual(len({(x["slug"], x["lang"]) for x in expected}), 48)
        ...
        self.assertEqual(sql.count("ON CONFLICT(slug) DO UPDATE SET cover_url='',status='published'"), 16)
