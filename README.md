...
- 🎓 **Free courses** — Go (50 lessons), Python (55 lessons) and SQL (13 lessons), plus **Rust: from zero to your own organizer**, released five lessons at a time in Kazakh, Russian and English. The first Rust publication contains lessons 1–5 and a preface; lessons 6–10 are prepared for the second release. Sources live in [`course/lessons/`](course/lessons/); Rust examples and exercise answers are checked on Windows, macOS and Linux. Lessons belong to course contents and are excluded from the main article feed. See the [Rust publication procedure](docs/rust-publication.md).
...
- 📍 **Advertising priced by reach** — an advertiser buys a place, not the whole country, and pays by the square root of the population share it covers: 135,000 ₸ nationwide for a month of the top banner, 27,000 ₸ for one region, 2,889 ₸ for a village. The card is anchored at half what a comparable outlet charges for a comparable placement. The banner runs on that place's pages and on everything inside it. No impression count is promised and no reader is located — the targeting is by what a page is about.
- 📣 **Block-resilient syndication** — RSS always on; Telegram carries the channel and the bot separately. A channel cannot show two subscribers different things, so it takes only what is written for everyone; anything addressed to a place goes through the bot, which asks the reader once where they live and then delivers by the same containment rule the site's feed uses — a region's news reaches everyone inside it, a village's reaches that village alone.
