...
func TestAdFormats(t *testing.T) {
	...
	if fs[0].Code != "horizontal" || fs[0].Price30 != 90000 {
		t.Errorf("first format should be horizontal @90000, got %s @%d", fs[0].Code, fs[0].Price30)
	}
...
func TestAdOrderTotal(t *testing.T) {
	// horizontal 30d rate = 90000; home+realestate weight = 20+20 = 40 (×10).
	// total = 90000 * 40 / 10 = 360000.
	p := AdOrderTotal("horizontal", []string{surfaceHome, surfaceRealestate}, 30)
	if p.Total != 360000 {
		t.Errorf("total = %d, want 360000", p.Total)
	}
	if p.Surfaces != 2 || p.Weight10 != 40 || p.FormatRate != 90000 {
	...
	if p.FormatRate != 8000 { // rectangle 3-day
		t.Errorf("unknown duration should use the 3-day rate 8000, got %d", p.FormatRate)
	}
}

func TestAdSurfaceFormatPrice(t *testing.T) {
	// horizontal 30d on home: 90000 * 20 / 10 = 180000.
	if got := AdSurfaceFormatPrice("horizontal", surfaceHome, 30); got != 180000 {
		t.Errorf("= %d, want 180000", got)
	}
	// Unknown format → 0.
