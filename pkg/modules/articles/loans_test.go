...
func TestLoanTotalCostIsTheHeadlineFigure(t *testing.T) {
	...
	if p.Loan != 16_000_000 {
		t.Fatalf("сумма кредита %d, ожидалось 16 000 000", p.Loan)
	}
	if p.Monthly < 246_000 || p.Monthly > 247_500 {
		t.Errorf("платёж %d ₸, ожидалось около 246 930", p.Monthly)
	}
	if p.Multiple < 3.1 || p.Multiple > 3.3 {
		t.Errorf("покупка обошлась в %.2f× цены, ожидалось около 3,2×", p.Multiple)
	}
	if p.TotalCost != 4_000_000+p.TotalPaid {
		t.Errorf("итог %d не равен взнос плюс выплаты", p.TotalCost)
	}
	if p.Overpay != p.TotalPaid-p.Loan {
		t.Errorf("переплата %d не равна выплаты минус тело кредита", p.Overpay)
	}
}
...
func TestDifferentiatedCostsLessButStartsHeavier(t *testing.T) {
	...
	if dif.Interest >= ann.Interest {
		t.Errorf("дифференцированный дал процентов %d, аннуитет — %d; должен быть дешевле", dif.Interest, ann.Interest)
	}
	if dif.Monthly <= ann.Monthly {
		t.Errorf("первый платёж дифференцированного %d не больше аннуитетного %d", dif.Monthly, ann.Monthly)
	}
	if dif.LastMonth >= dif.Monthly {
		t.Errorf("последний платёж %d не меньше первого %d — платёж должен убывать", dif.LastMonth, dif.Monthly)
	}
	...
	for _, p := range []LoanPlan{ann, dif} {
		...
		if principal != p.Loan {
			t.Errorf("погашено тела кредита %d при сумме %d", principal, p.Loan)
		}
		if last := p.Schedule[len(p.Schedule)-1].Balance; last != 0 {
			t.Errorf("после последнего платежа остаток %d, ожидался ноль", last)
		}
	}
}
...
func TestFeesPushTheEffectiveRateAboveTheAdvertisedOne(t *testing.T) {
	...
	if math.Abs(clean.EffRate-21.94) > 0.15 {
		t.Errorf("без комиссий эффективная ставка %.2f %%, ожидалось 21,94 — 20 %% с ежемесячным начислением", clean.EffRate)
	}
	if clean.EffRate <= 20 {
		t.Errorf("эффективная ставка %.2f %% не выше объявленных 20 %% — начисление по месяцам не учтено", clean.EffRate)
	}
	...
	if loaded.EffRate <= clean.EffRate+3 {
		t.Errorf("комиссии подняли ставку с %.2f лишь до %.2f — расчёт их не учитывает", clean.EffRate, loaded.EffRate)
	}
	if loaded.BankFees <= 0 || loaded.Insurance <= 0 {
		t.Errorf("комиссии банка %d и страховка %d должны быть разделены", loaded.BankFees, loaded.Insurance)
	}
}
...
func TestSharesAccountForEveryTenge(t *testing.T) {
	...
	if sum != p.TotalCost {
		t.Errorf("получатели в сумме дают %d, а покупатель отдаёт %d", sum, p.TotalCost)
	}
	if p.Shares.Seller != 20_000_000 {
		t.Errorf("продавцу досталось %d, а цена — 20 000 000", p.Shares.Seller)
	}
}
...
func TestSubsidyIsWhatTheBudgetAdds(t *testing.T) {
	...
	if p.Shares.Subsidy <= 0 {
		t.Fatal("для льготной ставки бюджетная доплата не посчитана")
	}
	...
	if want := market.Interest - p.Interest; p.Shares.Subsidy != want {
		t.Errorf("доплата %d, ожидалось %d — разница процентов по рынку и по льготе", p.Shares.Subsidy, want)
	}
	...
	if q := LoanCalc(LoanInput{Kind: LoanMortgage, Price: 20_000_000, Down: 4_000_000,
		...
		t.Errorf("без рыночной ставки посчитана доплата %d", q.Shares.Subsidy)
	}
}
...
func TestHalfEnteredInputGivesAnEmptyPlan(t *testing.T) {
	cases := []struct {
		...
		{"цена не введена", LoanInput{Kind: LoanMortgage, Months: 240, RatePct: 18}},
		{"срок не введён", LoanInput{Kind: LoanMortgage, Price: 20_000_000, RatePct: 18}},
		{"отрицательная цена", LoanInput{Kind: LoanMortgage, Price: -5, Months: 240, RatePct: 18}},
		{"взнос больше цены", LoanInput{Kind: LoanMortgage, Price: 20_000_000, Down: 30_000_000, Months: 240, RatePct: 18}},
		{"отрицательная ставка", LoanInput{Kind: LoanConsumer, Amount: 1_000_000, Months: 12, RatePct: -5}},
		{"срок за пределом", LoanInput{Kind: LoanConsumer, Amount: 1_000_000, Months: 99999, RatePct: 10}},
	}
	for _, c := range cases {
		p := LoanCalc(c.in)
		if p.Monthly < 0 || p.TotalPaid < 0 || p.Loan < 0 ||
			...
			t.Errorf("%s: бессмысленный расчёт %+v", c.name, p)
		}
		if len(p.Schedule) > 600 {
			t.Errorf("%s: график на %d месяцев", c.name, len(p.Schedule))
		}
	}
}
...
func TestZeroRateRepaysInEqualParts(t *testing.T) {
	...
	if p.Monthly != 100_000 {
		t.Errorf("платёж %d, ожидалось 100 000", p.Monthly)
	}
	if p.Overpay != 0 || p.EffRate != 0 {
		t.Errorf("под ноль процентов переплата %d и ставка %.2f, ожидались нули", p.Overpay, p.EffRate)
	}
}
...
func TestProgramRateReadsBasisPoints(t *testing.T) {
	if got := (LoanProgram{RateBP: 700}).Rate(); got != 7 {
		t.Errorf("700 базисных пунктов прочитаны как %v %%, ожидалось 7", got)
	}
	if got := (LoanProgram{RateBP: 1875}).Rate(); got != 18.75 {
		t.Errorf("1875 базисных пунктов прочитаны как %v %%, ожидалось 18,75", got)
	}
}
