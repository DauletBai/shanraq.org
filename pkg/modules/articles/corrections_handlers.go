...
func fmtT(lang, key string, args ...any) string {
	return fmt.Sprintf(site.T(lang, key), args...)
}
...
func (m *Module) decideCorrection(ctx context.Context, a *Article, tr *Translation, lang string, c Correction) (string, string, string) {
	...
	if n, err := m.corrections.CountSince(ctx, c.Reporter, time.Now().Add(-24*time.Hour)); err == nil && n >= correctionsPerDay {
		return CorrFailed, "", site.T(lang, "corr.e_rate")
	}
	...
	if st, found, _ := m.corrections.Duplicate(ctx, a.ID, c.Reporter, lang, c.Word); found && st == CorrApplied {
		return CorrApplied, "", site.T(lang, "corr.already")
	}

	id, err := m.corrections.Insert(ctx, c)
	if err != nil {
		m.rt.Logger.Error("correction insert", zap.Error(err))
		return CorrFailed, "", site.T(lang, "corr.e_internal")
	}
	sid := id.String()

	// Tier 1 — structure. Where in the article did they mean?
	cs, ok := findCorrectionSite(tr.BodyMD, c.Sentence, c.Word)
	if !ok {
		return reject(sid, "corr.e_not_found")
	}

	// Tier 2 — mechanics. A Latin letter wearing a Cyrillic face needs no model.
	if fixed, ok := repairHomoglyphs(tr.BodyMD[cs.Start:cs.End]); ok {
		if err := safeFix(tr.BodyMD[cs.Start:cs.End], fixed); err == nil {
			return m.commitCorrection(ctx, sid, a, tr, lang, cs, fixed)
		}
	}
	...
	word := tr.BodyMD[cs.Start:cs.End]
	v, err := m.ai.Proofread(ctx, lang, cs.Para, c.Sentence, word)
	switch {
	...
	if !v.Fix {
		reason := v.Reason
		if reason == "" {
			reason = site.T(lang, "corr.r_no_error")
		}
	...
	stands, why, err := m.ai.ProofreadRefute(ctx, lang, cs.Para, c.Sentence, word, fixed)
	if err != nil {
...
func (m *Module) commitCorrection(ctx context.Context, id string, a *Article, tr *Translation, lang string, cs correctionSite, fixed string) (string, string, string) {
	was := tr.BodyMD
	word := was[cs.Start:cs.End]
	updated := applyAt(was, cs, fixed)
	...
	if err != nil {
		...
		return CorrPending, fixed, site.T(lang, "corr.r_held")
	}
	if !okWrite {
		...
		m.recordCorrection(ctx, id, CorrPending, fixed, "")
		return CorrPending, fixed, site.T(lang, "corr.r_stale")
	}
	m.recordCorrection(ctx, id, CorrApplied, fixed, "")
	m.notifyAuthorOfCorrection(ctx, a, lang, word, fixed, cs)
...
func (m *Module) holdCorrection(ctx context.Context, id, lang string) (string, string, string) {
	...
	return CorrPending, "", site.T(lang, "corr.r_held")
}
...
func (m *Module) notifyAuthorOfCorrection(ctx context.Context, a *Article, lang, word, fixed string, cs correctionSite) {
	if m.mailer == nil || strings.TrimSpace(a.AuthorEmail) == "" {
		return
	}
	url := m.rt.Config.PublicBaseURL + "/read/" + a.Slug + "?lang=" + lang
	subject := site.T(a.OriginalLang, "corr.mail_subject")
	body := strings.Join([]string{
		fmtT(a.OriginalLang, "corr.mail_intro", url),
		"",
		fmtT(a.OriginalLang, "corr.mail_change", word, fixed),
		"",
		site.T(a.OriginalLang, "corr.mail_where") + " " + cs.Para,
		"",
		site.T(a.OriginalLang, "corr.mail_undo"),
	}, "\n")
