...
// renderLessonTOC renders a lesson and folds its answers. The rendering itself
// is the site's (site.RenderMarkdownTOC); what is folded, and when, is the
// course's business and stays here.
func renderLessonTOC(source string) (template.HTML, []site.TOCItem) {
	html, toc := site.RenderMarkdownTOC(source)
	return template.HTML(foldAnswers(string(html))), toc //nolint:gosec // already rendered by goldmark without raw HTML
}
