...
**Обязательное.** Напишите программу, которая рядом с собой создаёт файл со своими числами (год и значение через точку с запятой), читает его построчно через `with` и `encoding="utf-8"`, разбирает строки и пишет отчёт во второй файл. Пути стройте от `Path(__file__).parent`.
