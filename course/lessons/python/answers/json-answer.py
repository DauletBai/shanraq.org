...
for value in series.values():
    total += value
print(f"лет с числом: {len(series)}, среднее: {total / len(series):.2f}")
print("пропуски:", gaps)

text = json.dumps(series, ensure_ascii=False)
back = json.loads(text)
print("ключ до:", type(list(series)[0]).__name__, "| после:", type(list(back)[0]).__name__)

fixed = {}
for key, value in back.items():
    fixed[int(key)] = value
print("после починки:", fixed[2025])
