"""The answer to lesson 21's exercise, Kazakh: a history that survives a second run.
...
@dataclass
class Rate:
    """Тарихтың бір жолы: күн, валюта, бағам."""
...
def save(records):
    """Жазбаларды дерекқорға салады: жаңасы қосылады, танысы жаңарады."""
    conn.executemany("""
...
save(rows)
print("бірінші іске қосудан кейін:", conn.execute("SELECT count(*) FROM rates").fetchone()[0])
save(rows)
print("екінші іске қосудан кейін:", conn.execute("SELECT count(*) FROM rates").fetchone()[0])

print("доллар күндер бойынша:")
