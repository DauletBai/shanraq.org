"""The answer to lesson 14's exercise: three measurements on a time axis.
...
span = days[-1] - days[0]
print("дней между первым и последним:", span.days)
print(f"средний промежуток: {span.days / (len(days) - 1):.2f} дня")
print("следующий замер:", (days[-1] + timedelta(days=30)).strftime("%d.%m.%Y"))

almaty = timezone(timedelta(hours=5))
moment = datetime.strptime(report, "%d.%m.%Y %H:%M").replace(tzinfo=almaty)
print("отчёт по Алматы:", moment.isoformat())
print("он же в UTC:    ", moment.astimezone(timezone.utc).isoformat())
