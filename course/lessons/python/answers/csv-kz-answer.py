...
print("split(','):", len(line.split(",")), "кесек")
with data.open(encoding="utf-8", newline="") as source:
    print("csv.reader:", len(list(csv.reader(source))[2]), "өріс")
...
with data.open(encoding="utf-8", newline="") as source:
    for row in csv.DictReader(source):
        if row["price"] == "n/a":
            print(f"{row['month']}: олқылық — {row['note']}")
            continue
        total += float(row["price"].replace(",", "."))
        count += 1
        print(f"{row['month']}: {row['price']} — {row['note']}")
print(f"алынған ай: {count}, орташа {total / count:.2f}")
