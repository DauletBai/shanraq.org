...
for value in series.values():
    total += value
print(f"саны бар жыл: {len(series)}, орташа: {total / len(series):.2f}")
print("олқылықтар:", gaps)

text = json.dumps(series, ensure_ascii=False)
back = json.loads(text)
print("кілт дейін:", type(list(series)[0]).__name__, "| кейін:", type(list(back)[0]).__name__)

fixed = {}
for key, value in back.items():
    fixed[int(key)] = value
print("жөндегеннен кейін:", fixed[2025])
