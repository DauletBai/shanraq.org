"""The answer to lesson 22's exercise, Kazakh: the report the database writes itself.
...
conn.executemany("INSERT INTO currencies VALUES (?, ?, ?)", [
    ("USD", "АҚШ доллары", 1), ("UZS", "өзбек сомы", 100),
])
conn.commit()

print("== валюталар бойынша")
for row in conn.execute("""
    ...
    ORDER BY code
"""):
    print(f"{row['code']}: {row['days']} күн, {row['low']}-тан {row['high']}-ке дейін, орташа {row['mid']}")

print("== бір күнде, атауларымен")
for row in conn.execute("""
    ...
    ORDER BY unit DESC
""", ("2026-01-16",)):
    print(f"{row['name']}: бір бірлігі {row['unit']}")

print("== доллар айлар бойынша")
for row in conn.execute("""
...
print("фунт:", "дерек жоқ" if mid is None else round(mid, 2))
