"""The answer to lesson 19's exercise: the same answer, read by a parser.
...
print("дата:", root.findtext("date"), "| валют в ответе:", len(root.findall("item")))
...
for item in root.findall("item"):
    ...
    print(f"{code}: {rate} за {quant} → {rate / quant:.4f} тенге за единицу")

first = root.find("item")
print("автора у ответа нет:", first.findtext("author", "тега author нет"))
