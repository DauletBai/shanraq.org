"""The answer to lesson 14's exercise, Kazakh: three measurements on a time axis.
...
span = days[-1] - days[0]
print("бірінші мен соңғының арасындағы күн:", span.days)
print(f"орташа аралық: {span.days / (len(days) - 1):.2f} күн")
print("келесі өлшеу:", (days[-1] + timedelta(days=30)).strftime("%d.%m.%Y"))

almaty = timezone(timedelta(hours=5))
moment = datetime.strptime(report, "%d.%m.%Y %H:%M").replace(tzinfo=almaty)
print("Алматы бойынша есеп:", moment.isoformat())
print("сол UTC-де:          ", moment.astimezone(timezone.utc).isoformat())
