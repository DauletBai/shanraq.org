"""The answer to lesson 22's exercise, English: the report the database writes itself.
...
conn.executemany("INSERT INTO currencies VALUES (?, ?, ?)", [
    ("USD", "the US dollar", 1), ("UZS", "the Uzbek sum", 100),
])
conn.commit()
...
for row in conn.execute("""
    ...
    ORDER BY code
"""):
    print(f"{row['code']}: {row['days']} days, from {row['low']} to {row['high']}, average {row['mid']}")

print("== one day, with names")
for row in conn.execute("""
    ...
    ORDER BY unit DESC
""", ("2026-01-16",)):
    print(f"{row['name']}: {row['unit']} for one")

print("== the dollar by month")
for row in conn.execute("""
...
print("the pound:", "no data" if mid is None else round(mid, 2))
