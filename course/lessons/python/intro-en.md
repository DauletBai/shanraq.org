...
The web is covered: the [Go course](https://shanraq.org/course/go?lang=en) takes a reader to a blog of their own, live on the internet at their own domain, with a database, sessions, HTTPS and a service that survives a reboot. Repeating that in another language would add no skill.
...
1. [Counting for yourself: prices, the tenge and one question](https://shanraq.org/read/py-agany-bagalar-tenge?lang=en) — a program that works with nothing installed.
2. [A workplace: Python, the project's environment and the first run](https://shanraq.org/read/py-jumys-orny-venv?lang=en) — installing the language and making an environment.

Or open the [course map](https://shanraq.org/course/python?lang=en) in full — the whole path to your own digest is visible there.
