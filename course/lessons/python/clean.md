...
Пробелы внутри числа убираются до перевода: `str.replace(" ", "", regex=False)`. Там же снимаются валюта, знак процента и неразрывный пробел, который приходит из Excel и выглядит как обычный.
...
- ряд по времени, где значение держится до следующего изменения (`ffill`): курс на выходной равен курсу пятницы, потому что он действительно ему равен;
...
3. Честно — когда заполняется настоящее значение по умолчанию, когда ряд по времени держит прежнее значение до следующего изменения, и когда вместо числа ставится метка «неизвестно». Выдумка — когда пропуск заполняют средним или нулём, чтобы формула посчиталась: такие данные потом не отличить от настоящих.
