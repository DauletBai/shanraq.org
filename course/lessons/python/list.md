...
1. `260 1890 3 [260, 620]`. Счёт с нуля, `-1` — последний элемент, срез `[:2]` берёт два первых и не включает второй индекс.
...
2. `sum(prices) / len(prices)`. Обе функции берут список целиком, поэтому средняя цена считается одной строкой.
...
3. `IndexError: list index out of range`. В списке три элемента, значит индексы `0`, `1`, `2`, а `3` — уже за краем. Последний берут через `-1`, и тогда номер не придётся пересчитывать при каждом изменении списка:
