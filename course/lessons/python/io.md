...
Пропуск, в отличие от них, переживает дорогу: пустая ячейка в файле читается как `NaN`, а `NaN` пишется пустой ячейкой. Именно поэтому в примере пустых ячеек ровно две — те, что были.
...
Правило: **JSON от чужого API читайте `json.load`, JSON от собственного `to_json` — `read_json`**.
...
Два предупреждения. Значения в запрос подставляют через `params=`, а не форматированием строки — почему, показано там же, в уроке про SQLite. И `sqlite3` — единственная база, с которой pandas работает напрямую; для PostgreSQL и прочих нужен SQLAlchemy, но сам вызов выглядит так же.
