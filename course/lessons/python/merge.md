...
тот же, а вот что произойдёт с строками, которым пары не нашлось, зависит от
того, как соединять.
"""
...
Это самая частая тихая потеря данных в работе с таблицами. Отчёт выходит правдоподобным, суммы меньше настоящих, и заметить это можно только одним способом — считать строки до и после.

Привычка: **после каждого соединения сравнивать длину**. `len(data)` и `len(joined)` разошлись — значит, либо потеряли, либо размножили; третьего не бывает.
...
`merge` соединяет **по ключу**, добавляя столбцы. `concat` складывает таблицы **друг под друга** (или рядом), не спрашивая ни о каком ключе: так собирают двенадцать месячных файлов в один ряд. Если вы ищете «объединить таблицы», сначала решите, что именно вам нужно: больше столбцов или больше строк.
...
Готово, когда: вывод совпадает построчно; число строк до и после одинаковое; соединение проверено через `validate`; строки без региона не попали в группировку по регионам и не были приписаны к чужому региону.
...
2. `merge(..., how="outer", indicator=True)` добавляет столбец `_merge` со значениями `both`, `left_only` и `right_only`; `value_counts()` по нему и даёт три числа.
