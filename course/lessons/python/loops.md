...
1. `23.0`. `continue` бросает текущий круг и идёт к следующему значению, поэтому `None` не попадает в сумму, а `8.0 + 15.0` — попадают.
...
2. `range(2021, 2026)`. Правая граница не входит, поэтому последний год пишут на единицу больше, чем нужен, — это первое место, где новичок теряет год.
...
3. `range(1, len(years))` начинает со второго элемента: у списка счёт с нуля. Когда нужен сам элемент, а не его номер, по списку идут напрямую:
