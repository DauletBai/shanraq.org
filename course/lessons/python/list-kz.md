...
Ереже біреу: **сол шек кіреді, оң шек кірмейді**. `prices[1:3]` 1 мен 2 нөмірлі элементтерді береді, 3-ті емес. Бұл `prices[:3]` пен `prices[3:]` бірге тұтас тізімді қабаттаспай, жетіспейтін мәнсыз берсін деп жасалған.
...
2. `sum(prices) / len(prices)`. Екі функция да тізімді тұтас алады, сондықтан орташа баға бір жолмен саналады.
...
3. `IndexError: list index out of range`. Тізімде үш элемент бар, демек индекстер `0`, `1`, `2`, ал `3` — шеттен тыс. Соңғысын `-1` арқылы алады, сонда тізім өзгерген сайын нөмірді қайта санаудың қажеті болмайды:
