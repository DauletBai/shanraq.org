...
Причина простая: R² сравнивает модель с самой слабой из возможных — с горизонтальной прямой на уровне среднего. Обыграть её легко у любого ряда, который растёт. Поэтому R² почти никогда не бывает ответом на вопрос «хороша ли модель».
...
Вот это — настоящий диагноз. Если модель верна по форме, её промахи разбросаны случайно: знак меняется часто, групп примерно вдвое меньше, чем точек. Здесь групп три: модель сначала систематически завышает, потом столько же лет занижает, потом опять завышает.
