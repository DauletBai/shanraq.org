...
2. `"utf-8-sig"`. Обычный `utf-8` оставит метку Excel приклеенной к имени первой колонки, и `row["year"]` ответит `KeyError` при живой колонке.
...
3. `",".join(row)` записал запятые примечания как разделители, и при чтении полей стало пять вместо трёх. Кавычки ставит писатель — `csv.writer`:
