# Итераторы и генераторы: четыре мегабайта против четырёхсот байт

_Лид (summary):_ **Шестнадцатый урок курса по Python. Сто тысяч квадратов списком занимают почти четыре мегабайта, генератором — 376 байт: измерено, вместе со всеми числами. Разница не в том, как считать, а в том, когда появляются значения. Плюс главная ошибка: генератор одноразовый, и второй проход по нему тихо даёт пустоту.**
...
# занято на самом деле, считает tracemalloc.
tracemalloc.start()
...
tracemalloc.stop()
print("список:    сам объект", sys.getsizeof(squares_list), "байт, со всеми числами", list_all)
print("генератор: сам объект", sys.getsizeof(squares_gen), "байт, со всем, что держит", gen_all)
...
список:    сам объект 800984 байт, со всеми числами 3999680
генератор: сам объект 208 байт, со всем, что держит 376
