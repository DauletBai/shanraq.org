...
У вас числа будут другие: другая машина, другая версия, другой фон. Важны не сами числа, а расстояния между колонками — они одинаковые везде.
...
**Медленнее — там, где мы попросили лишнего.** `df.loc[df["kind"] == "еда", "amount"].sum()` сначала строит маску: миллион ответов «да/нет», целый новый столбец, — и только потом складывает. А `sum(a for k, a in rows if k == "еда")` проходит один раз и не строит ничего. На одном вопросе список выигрывает. На десяти вопросах к той же таблице маска строится один раз, а вопросов десять, — и выигрывает уже pandas.
