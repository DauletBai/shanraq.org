...
1. `which python3` покажет системный Python — `/usr/bin/python3` или похожий, но не путь внутри `.venv`. Значит, и `pip install requests` поставит пакет в систему: он окажется у всех проектов сразу, а в `requirements.txt` не попадёт, потому что его туда никто не записывал.
