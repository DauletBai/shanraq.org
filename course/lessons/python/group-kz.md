...
print(by_two)
print("қолтаңбалар:", type(by_two.index).__name__)
...
print()
print("== count сандарды санайды, size — жолдарды")
with_gap = df.copy()
...
== count сандарды санайды, size — жолдарды
...
Айырмашылық сырттай ғана емес. `count` `NaN`-ды өткізіп жібереді, `size` — жоқ, ал екеуінің арасындағы айырма — топтағы жетіспейтін мәндердің саны. Мысалда бір мән өшірілгеннен кейін «тамақта» `count` 3, ал `size` 4 береді.
...
### Екі кілт және екі қабатты қолтаңба

`groupby(["city", "kind"])` `MultiIndex` береді: жол қолтаңбасы жұптан тұрады. Оны қарауға ыңғайлы, әрі әрі қарай беруге ыңғайсыз, сондықтан көбіне бірден `.reset_index()` жазылады — сонда кілттері екі баған болып тұрған кәдімгі жазық кесте шығады.
...
Бүкіл әңгіме осы пішін үшін еді: есеп елдер бойынша циклдің орнына бір сұраққа айналады. `groupby("country").agg(...)` жылдарды, жетіспейтін мәндерді, орташаны және ең жоғарысын береді, ал ең жоғарының жылы топтаудың ішіндегі `idxmax`-тан келеді — ең үлкен мәннің қолтаңбасы сол жолдың өзі, ал жол өз жылын біледі.
...
1. Кестені кілт бойынша бөліктерге кеседі, әр бөлікті бөлек санайды және жауаптарды қайта кестеге жинайды. Кілт жол қолтаңбасына — индекске — айналады, сондықтан жауапты сұрыптауға және сол қолтаңбалар бойынша басқа қатарлармен қосуға болады.
2. `count` бар мәндерді санайды, `size` — жолдарды, жетіспейтіндерін қоса. Олардың айырмасы — топтағы жетіспейтін мәндердің саны.
