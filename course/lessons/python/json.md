...
print("по умолчанию:  ", json.dumps({"note": "дерек жоқ"}))
...
Для файлов, которые открывают глазами, пишут `ensure_ascii=False`, а для читаемости добавляют `indent=2`. Для передачи по сети — наоборот, оставляют как есть: короче.
