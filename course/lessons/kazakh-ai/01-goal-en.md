...
Question → identify the request → find a record → answer from that record. If the request is unclear or no record exists → “білмеймін” (“I don't know”). The arrow means “move to the next action”; it is not part of the Kazakh text.
...
Hide the map and name its four steps. Explain why a time from another class cannot fill a missing record.

## Task

Records: “Robotics — Tuesday, 16:00”; “Chess — Thursday, 15:00.” For each question, give a source and answer or a reason to withhold an answer: 1) “Шахмат сабағы қашан?” (“When is chess class?”) 2) “Сурет сабағы қашан?” (“When is art class?”) 3) “Сабақ қашан?” (“When is the class?”). The last question omits the class name.

**Hint:** find the class name first. **Check:** 1) Chess record, Thursday 15:00; 2) no art record, so its time is unknown; 3) ask which class. Continue when you can distinguish a supported answer from a guess.
