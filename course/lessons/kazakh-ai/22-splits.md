...
Новое поле `split` указывает конверт: `train` — обучение, `tune` — настройка, `test` — контроль. В файле 8, 5 и 6 вопросов соответственно. Состав вопросов распределён вручную, без случайности: результат можно повторить. Один и тот же текст не должен попасть в разные конверты. `seen` — список уже прочитанных текстов; `valid` запоминает, обнаружена ли ошибка. `True` значит «верно», `False` — «неверно».
