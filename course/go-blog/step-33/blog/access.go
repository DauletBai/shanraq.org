...
// not.
func (s *Store) MayEdit(ctx context.Context, user User, slug string) error {
	if user.ID == 0 {
		return ErrNeedLogin
	}

	var author sql.NullInt64
	err := s.db.QueryRowContext(ctx, `select author_id from articles where slug = ?`, slug).Scan(&author)
	switch {
