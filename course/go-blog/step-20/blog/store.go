...
func (s *Store) Get(slug string) (Article, error) {
	...
	err := s.db.QueryRow(
		...
		Scan(&a.Slug, &a.Title, &a.Words, &a.Lang, &a.Body, &updated)
	switch {
...
func (s *Store) All(sort string) ([]Article, error) {
	order, ok := sorts[sort]
	if !ok {
		order = sorts[""]
	}
	rows, err := s.db.Query(
		`select slug, title, words, lang, body, updated_at from articles order by ` + order)
	if err != nil {
