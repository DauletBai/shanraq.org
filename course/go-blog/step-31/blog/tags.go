...
// that stays for ever.
func (s *Store) AddWithTags(ctx context.Context, a Article, tags []string) error {
	...
	res, err := tx.ExecContext(ctx,
		`insert into articles (slug, title, words, lang, body, author_id, cover)
		 values (?, ?, ?, ?, ?, ?, ?)`,
		a.Slug, a.Title, a.Words, a.Lang, a.Body, nullID(a.AuthorID), a.Cover)
	if err != nil {
	...
	for _, name := range tags {
		...
		// hand the id back.
		if err := tx.QueryRowContext(ctx, `insert into tags (name) values (?)
...
func (s *Store) TagsOf(ctx context.Context, slug string) ([]string, error) {
...
func (s *Store) ByTag(ctx context.Context, tag string) ([]Article, error) {
	rows, err := s.db.QueryContext(ctx, `select a.slug, a.title, a.words, a.lang, a.body, a.updated_at
		from articles a
