"""Step 23 -- after lesson 44, the model is asked about a year it did not see.
...
New in this step: the trend is taught a second time without the last year and
then asked about it. The error on a year the model did not see stands on the
page next to the error on the years it learned from, because they are not the
same number and the smaller one is the one that flatters. Still no forecast: one
held-out year is a start, not a verdict.
