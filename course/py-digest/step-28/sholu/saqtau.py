...
def save(table, path):
    ...
    The numbers go to disk as they came, not rounded to two decimals as they
    once were. Two decimals look like plenty for a rate of inflation, and they
    are -- until a trend is fitted through sixteen of them and the report says
    3.55 on the run that fetched and 3.56 on the run that read the file back.
    A store that changes the answer is not a store.
    """
    return _replace(table.to_csv(index=False), path)
