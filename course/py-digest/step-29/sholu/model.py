...
def route_claim(claim_type, evidence_ids, known_ids, formula_id=None):
    """Only observations enter automatically; causes wait for a person."""
...
def trust_note(source):
    """A visible refusal assembled from policy, not model prose."""
    decision = route_claim("cause", [source["source_id"]], {source["source_id"]})
    assert decision == "review"
    return "Себеп туралы тұжырым автоматты жарияланбады: оған бөлек дәлел мен адам тексеруі керек."
