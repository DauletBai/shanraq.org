...
<style>
 ...
 .about { color: #5c5c5c; font-size: 13px; border-top: 1px solid #d6d3ce; margin-top: 24px; padding-top: 12px; }
 .about dt { font-weight: 600; }
 .about dd { margin: 2px 0 10px; }
 .limits { border-top: 1px solid #d6d3ce; margin-top: 24px; padding-top: 12px; }
 .limits li { color: #5c5c5c; font-size: 13px; }
</style>
<h1>$title</h1>
...
FORMATS = {
    ...
    "байланыс": pishim.number,
    "жылдық": pishim.number,
    "ең үлкен қате": pishim.number,
    "соңғы жылда": pishim.number,
}
...
def _about(notes):
    """The indicators' own definitions, as a list at the foot of the page.

    A number without its definition is a number the reader has to guess at: an
    annual rate can be an average over the year or December against December,
    and the difference is several points. So the source's own words go on the
    page, under everything they explain.
    """
    if not notes:
        return ""
    rows = []
    for note in notes:
        head = html.escape(f"{note['код']} — {note['аты']}")
        body = html.escape(note["анықтама"])
        if note.get("дереккөз"):
            body += f" ({html.escape(note['дереккөз'])})"
        rows.append(f"<dt>{head}</dt>\n<dd>{body}</dd>")
    return '<dl class="about">\n' + "\n".join(rows) + "\n</dl>"


def _limits(lines, title):
    """What the report does not say, as a list under everything it does say."""
    if not lines:
        return ""
    items = "\n".join(f"<li>{html.escape(line)}</li>" for line in lines)
    return f'<div class="limits">\n<p><b>{html.escape(title)}</b></p>\n<ul>\n{items}\n</ul>\n</div>'


def write(report, picture, path, title, source, spread=None, index=None,
          money=None, money_source="", link=None, trend=None, limits=None,
          about=None, day=None, verified="", trust_note=""):
    """Writes the report, its picture, the three summary tables and the notes."""
    ...
    # Whether prices and money moved together, or -- on three countries -- the
    # sentence saying that cannot be answered here. Both are one row, because
    # a refusal the reader never sees is a refusal that teaches nothing.
    link_head, link_body = _table(link) if link is not None else ("", "")
    # The trend and what it misses by. No forecast: a line is worth one only
    # after it has been checked on years it did not see.
    trend_head, trend_body = _table(trend) if trend is not None else ("", "")
    ...
    page = PAGE.substitute(
        ...
        link_head=link_head,
        link_body=link_body,
        limits=_limits(limits or [], "Бұл есеп нені айтпайды"),
        trend_title="Тренд: көбейткіш, өз жылдарындағы қате және көрмеген жылдағы қате",
        trend_head=trend_head,
        trend_body=trend_body,
        verified=html.escape(verified),
        trust_note=html.escape(trust_note),
