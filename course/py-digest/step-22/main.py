"""Step 22 -- after lesson 43, the digest fits its first model.
...
New in this step: a straight line through the logarithm of each country's price
index -- the first model in this program. It prints the yearly multiplier the
line implies and the worst it misses by, in points of the index, and it prints
no forecast at all. A line is worth a forecast only after it has been checked on
years it did not see, and that is the next lesson.
...
def main(argv=()):
    ...
    log.append("тренд: " + ", ".join(
        f"{row['аты']} — {row['жылдық']} есе/жыл, қатесі {row['ең үлкен қате']}"
