...
trend() is lesson 43 and the first model in this program: a straight line
through the logarithm of the index, which is the shape prices actually have.
What it prints is the yearly multiplier the line implies and the worst it misses
by -- and no forecast at all. A model is worth a forecast only after it has been
checked on years it did not see, and that is the next lesson.
"""
...
def trend(table, minimum=5):
    ...
    thin, and the digest has exactly five.
    """
    assert not table.empty, "кестеде бірде-бір жол жоқ"
    rows = []
    for country, part in table.sort_values("year").groupby("country", observed=True):
        ...
        if len(years) < minimum:
            rows.append({"country": str(country), "жыл": len(years), "жылдық": None,
                         "ең үлкен қате": None, "қорытынды": f"аз: {len(years)} жыл"})
            continue
    ...
    named = out.merge(anyqtama.table(), on="country", how="left", validate="one_to_one")
    named["аты"] = named["аты"].fillna(named["country"])
    return named[["country", "аты", "жыл", "жылдық", "ең үлкен қате", "қорытынды"]]
